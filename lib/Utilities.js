...
export const isWhatsAppURL = (string) => {
   ...
   if (!string.includes('whatsapp.com')) return false

   return WHATSAPP_URL_REGEX.test(string)
}
...
export const resizeImage = async (media, width = 540, height = null, quality = 70) => {
...
export const findTopSuggestions = (input) => {
   ...
   for (const command in CommandIndex) {
      if (Math.abs(command.length - inputLength) > maxDistance) continue
...
export const Sender = (sock, updatePresence = true) => {
