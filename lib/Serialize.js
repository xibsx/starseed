...
import { areJidsSameUser, downloadContentFromMessage, getContentType, isJidGroup, isJidNewsletter, isJidStatusBroadcast, isLidUser, isPnUser, normalizeMessageContent } from '@itsliaaa/baileys'
...
export const findChatId = (key) => {
   if (isJidStatusBroadcast(key.remoteJid))
...
export const normalizeMentionedJid = (mentionedJid) => {
   ...
   for (let i = 0; i < mentionedSize; i++) {
      ...
      const normalized = isPnUser(userJid) ? userJid : userId
...
export const Serialize = (sock, message) => {
   if (message.key) {
      ...
      message.isStatus = isJidStatusBroadcast(message.chat)
   }
   ...
   if (message.quoted) {
      ...
      if (typeof message.quoted !== 'string') {
         ...
         quoted.isStatus = isJidStatusBroadcast(quoted.chat)
         quoted.key = {
   ...
   message.reply = (text, options = {}, extra = {}) =>
      sock.sendText(message.chat, text, message, options, extra)

   message.react = (text = '💛', extra = {}) =>
